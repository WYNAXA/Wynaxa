      ...
      <body className="min-h-full flex flex-col font-sans">
        ...
        <header className="sticky top-0 z-50 border-b border-gray-100 bg-white/90 backdrop-blur-md">
          <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
            <Link
              href="/"
              className="text-lg font-bold tracking-[0.2em] text-[#0B0F12] uppercase"
            >
              Wynaxa
            </Link>
            ...
            <nav className="hidden items-center gap-8 md:flex">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="text-sm text-gray-600 transition-colors hover:text-[#0B0F12]"
                >
                  {link.label}
                </Link>
              ))}
            ...
            {/* Mobile menu button */}
            <button
              className="md:hidden text-[#0B0F12]"
