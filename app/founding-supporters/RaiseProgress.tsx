...
export default function RaiseProgress({ raise }: { raise: RaiseData }) {
  ...
  const total = raise.received + raise.committed;
  ...
  return (
    <div className="mt-12 mb-12">
      ...
      <p className="text-3xl font-semibold text-foreground sm:text-4xl">
        {fmt(total)}{" "}
        <span className="text-gray-400 font-normal">of {fmt(raise.target)}</span>
      </p>

      {/* Caption */}
      <p className="mt-2 text-base text-gray-600">
        {fmt(raise.received)} received &middot; {fmt(raise.committed)} committed
        &middot; {raise.supporters} founding supporter{raise.supporters !== 1 ? "s" : ""}
      </p>
      ...
      >
        {/* Received segment — solid teal */}
        <div
          className="h-full rounded-l-full bg-accent transition-[width] duration-1000 ease-out"
          style={{ width: animated ? `${receivedPct}%` : "0%" }}
        />
        {/* Committed segment — teal at 40% opacity with solid border */}
        <div
          ...
          style={{
            ...
            borderRight: committedPct > 0 ? "1px solid #0E8C7F" : "none",
            borderRadius: receivedPct === 0 ? "9999px 9999px 9999px 9999px" : "0 9999px 9999px 0",
          }}
        />
      </div>
      ...
      <p className="mt-6 text-base text-gray-600">
        Our first supporter came in at &euro;4,000, followed by two of our board
        advisors. You would not be the first.
      </p>
    </div>
  );
}
